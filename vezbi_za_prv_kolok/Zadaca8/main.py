...
class ReshenaLista(Problem):
    def __init__(self,initial,n,l):
    ...
    def goal_test(self, state):
        ...
        ls=list(tapl)
        brojac=1
        for i in range(len(ls)-1,self.l-self.n-1,-1):
            if brojac != ls[i]:
                return False
            brojac+=1
        return True
    ...
    def successor(self, state):
        ...
        for i in range(len(ls)):
            if ls[i] == 0: continue
            for move,x in moves.items():
                if move=="R1" and i+1<self.l and ls[i+1] ==0:
                    ls1=list(tupl)
                    ls1[i+1]=ls1[i]
                    ls1[i]=0

                    succ[move+": Disk "+str(ls1[i+1])]=tuple(ls1)
                if move=="R2" and i+1<self.l and i+2<self.l and ls[i+1]!=0 and ls[i+2]==0:
                    ls1 = list(tupl)
                    ls1[i + 2] = ls1[i]
                    ls1[i] = 0

                    succ[move+": Disk "+str(ls1[i+2])] = tuple(ls1)
                if move=="L1" and i-1>=0 and ls[i-1] ==0:
                    ls1=list(tupl)
                    ls1[i-1]=ls1[i]
                    ls1[i]=0

                    succ[move+": Disk "+str(ls1[i-1])]=tuple(ls1)
                if move=="L2" and i-1>=0 and i-2>=0 and ls[i-1]!=0 and ls[i-2]==0:
                    ls1 = list(tupl)
                    ls1[i - 2] = ls1[i]
                    ls1[i] = 0
...
if __name__ == "__main__":
    ...
    tapl=tuple(lista)
    initial=tapl
    problem=ReshenaLista(initial,n,l)
