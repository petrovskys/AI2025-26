...
class Box(Problem):
    ...
    def goal_test(self, state):
        ...
        return (x1,y1) == (x2,y2) == (4,4)

    @staticmethod
    def check_valid(state):
        (x,y),(x1,y1),(x2,y2) = state
        return not (x,y) in ((x1,y1),(x2,y2)) and not (x1,y1) == (x2,y2)
    ...
    def successor(self, state):
        ...
        for move,(nx,ny) in moves.items():
            if not self.check_valid(state):
                continue
            if not ((nx+x<5 and nx+x>=0) and (ny+y<5 and ny+y>=0)):
                continue
            if (nx+x,ny+y) == (x1,y1) and (x1,y1) != (4,4):
                succ["Push box 1 " + move] = ((x+nx,y+ny),(x1+nx,y1+ny),(x2,y2))
                continue
            if (nx+x,ny+y) == (x2,y2) and (x2,y2) != (4,4):
                succ["Push box 2 " + move] = ((x+nx,y+ny),(x1,y1),(x2+nx,y2+ny))
                continue
            succ["Move man "+move] = (nx+x,ny+y),(x1,y1),(x2,y2)
        return succ
