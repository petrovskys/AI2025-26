...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        for move,(x,y) in Moves[direction].items():
            ls=list(snake)
            next_head=(x+ls[len(ls)-1][0],y+ls[len(ls)-1][1])
            if not (0<=next_head[0]<10 and 0<=next_head[1]<10):
                continue
            if next_head in apples:
                lsa = list(apples)
                lsa.remove(next_head)
                ls.append(next_head)
                succ[move]=tuple(ls),tuple(lsa),directions[direction][move]
            else:
                for i in range(len(ls)-1):
                    ls[i]=ls[i+1]
                ls[-1]=next_head
                succ[move]=tuple(ls),apples,directions[direction][move]
        return succ

    def h(self,node):
        snake, apples, _ = node.state
        if len(apples) == 0:
            return 0
        head = snake[-1]
        return sum(abs(head[0] - ax) + abs(head[1] - ay) for ax, ay in apples)
    def actions(self, state):
...
if __name__ == "__main__":
    ...
    snake=((0,9),(0,8),(0,7))
    initial = snake,tuple(zeleni_jabolki),"South"
