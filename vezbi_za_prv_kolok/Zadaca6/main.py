...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        for move,(nx,ny) in moves[nasoka].items():
            if not ((0<=Zmija[0][0]+nx<=9) and (0<=Zmija[0][1]<=9)):
                continue

            body=list(Zmija)
            body.remove(Zmija[0])

            if (Zmija[0][0]+nx,Zmija[0][1]+ny) in body:
                continue

            if (Zmija[0][0]+nx,Zmija[0][1]+ny) in self.cJ:
                continue

            if (Zmija[0][0]+nx,Zmija[0][1]+ny) in zJ:
                ls=list(Zmija)
                ls.insert(0,(Zmija[0][0]+nx,Zmija[0][1]+ny))
                ls1=list(zJ)
                ls1.remove((Zmija[0][0]+nx,Zmija[0][1]+ny))

                succ[move]=tuple(ls),tuple(ls1),movess[nasoka][move]
            else:
                ls=list(Zmija)
                for i in range(len(ls)-1,0,-1):
                    ls[i]=ls[i-1]

                ls[0]=(ls[0][0]+nx,ls[0][1]+ny)
                succ[move]=tuple(ls),zJ,movess[nasoka][move]
        return succ
...
if __name__ == '__main__':
    ...
    zJ=()
    for i in range(zJN):
        zJ+=(tuple(map(int,input().split(","))),)

    cJN=int(input())
    cJ=()
    for i in range(cJN):
        cJ+=(tuple(map(int,input().split(","))),)

    initial=((0,7),(0,8),(0,9)),zJ,"Jug"
    problem = Snake(initial,cJ)
