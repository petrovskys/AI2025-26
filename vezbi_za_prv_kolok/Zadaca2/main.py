...
class fudbal(Problem):
    def __init__(self,initial,goal,protivnik1,protivnik2):
        super().__init__(initial,goal)
        self.protivnik1=protivnik1
        self.protivnik2=protivnik2
    def goal_test(self, state):
        _,(x,y)=state
        return (x,y) in self.goal
    def check_valid(self,state):
        (Cx,Cy),(tX,tY)=state
        protivnik1=((3,3),(2,4),(3,4),(4,4),(2,3),(4,3),(2,2),(3,2),(4,2))
        protivnik2=((5,4),(4,4),(6,4),(4,3),(5,3),(6,3),(4,5),(5,5),(6,5))
        return (tX,tY) not in protivnik1 and (tX,tY) not in protivnik2
    ...
    def successor(self, state):
        ...
        for move,(x,y) in moves.items():
            if not self.check_valid(state):
                break
            if (Cx+x,Cy+y) in (self.protivnik1,self.protivnik2):
                continue
            if not ((0<=Cx+x<8) and (0<=Cy+y<6)):
                continue

            if (Cx + x, Cy + y) == (tX, tY):
                if not ((0 <= tX + x < 8) and (0 <= tY + y < 6)):
                    continue
                miniState = (Cx + x, Cy + y), (tX + x, tY + y)
                if self.check_valid(miniState):
                    succ["Push ball " + move] = (Cx + x, Cy + y), (tX + x, tY + y)
            else:
                succ["Move man " + move] = (Cx + x, Cy + y), (tX, tY)
...
if __name__ == '__main__':
    (chovekX,chovekY)=map(int,input().split(","))
    (topkaX,topkaY)=map(int,input().split(","))
    initial=(chovekX,chovekY),(topkaX,topkaY)
    problem=fudbal(initial,((7,2),(7,3)),(3,3),(5,4))
