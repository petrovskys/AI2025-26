...
class pillar(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(n):
            ...
            top_element=pillars[i][-1]
            for j in range(n):
                if i == j: continue
                if len(pillars[j]) == 0 or top_element <= pillars[j][-1]:
                    new_state = list(p for p in pillars)
                    new_state[i] = pillars[i][:-1]
                    new_state[j] = pillars[j] + (top_element,)
                    print(new_state[j])
                    succ["MOVE TOP BLOCK FROM PILLAR "+str(i+1)+" TO PILLAR "+str(j+1)]=tuple(new_state)
        return succ
    def actions(self, state):
...
if __name__ == "__main__":
    def parse(line):
        ...
        for p in line.split(";"):
            if p.strip() == '':
                ...
                pillars.append(tuple(map(int, p.split(","))))
    ...
    initial=parse(n)
    goal=parse(m)
    problem = pillar(initial,goal)
    solution = breadth_first_graph_search(problem)
    if solution is None:
        print("No Solution!")
    else:
        print("Number of action "+str(len(solution.solution())))
