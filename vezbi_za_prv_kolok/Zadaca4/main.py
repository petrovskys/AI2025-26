...
class igra(Problem):
    ...
    def check_valid(self,state,x,y,bx,by):

        topchinja=state
        if ( 0>x or x>=self.n ) or (y<0 or y>=self.n):
            return False
        if ((x,y) in topchinja or (x,y) in self.walls):
            return False
        if not (bx,by) in topchinja:
            return False
        return True
    ...
    def successor(self, state):
        ...
        br=0
        litopchinja = list(topchinja)
        for x,y in litopchinja:
            for move,(xt,yt) in moves.items():
                if self.check_valid(state,x+xt,y+yt,x+(xt//2),y+(yt//2)):
                    ls=litopchinja.copy()
                    ls[br]=(x+xt,y+yt)
                    ls.remove((x+(xt//2),y+(yt//2)))

                    succ[move+": (x="+str(x)+",y="+str(y)+")"]=tuple(ls)
            br+=1
...
if __name__ == "__main__":
    n=int(input())
    brojNaTopchinja=int(input())
    topchinja=()
    for i in range(brojNaTopchinja):
        topchinja+=(tuple(map(int,input().split(","))),)
    brNaDzidoj=int(input())
    dzidoj=()
    for i in range(brNaDzidoj):
            dzidoj+=(tuple(map(int,input().split(","))),)

    initial=topchinja
    problem=igra(initial,dzidoj,n)
